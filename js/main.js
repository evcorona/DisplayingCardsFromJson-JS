...
<div class="card p-2 m-1 bg-dark text-white border-0 col-12 col-md-4 col-lg-4 mt-3 shadow rounded">
  ...
  <ul class="list-group list-group-flush">
    <li class="list-group-item bg-dark text-white">Email: {email}</li>
    <li class="list-group-item bg-dark text-white">Teléfono: {telefono}</li>
    <li class="list-group-item bg-dark text-white">Rol: <b>{rol}</b></li>
  </ul>
</div>`

const fillData = (BDdata,html) => {
  BDdata.forEach( data => { 
    currentContent = document.getElementById("card").innerHTML
    let newCard = html.replace("imagen",data.pictureUrl).replace("name",data.name).replace("{email}",data.email).replace("{telefono}",data.phone).replace("{rol}",data.type)
    newContent = currentContent + newCard
    document.getElementById("card").innerHTML = newContent
  })
}

fillData (personArray,card)